...
class DSL():
    ...
    def __init__(self):
        ...
        self.val2attr = {'shape': ['rectangle', 'circle', 'triangle', 'square', 'pentagon', 'hexagon', 'octagon', 'star'],
...
class ProgramExecutor(DSL):
    ...
    def func_executor(self, func, param, prev_out):
        ...
        # 0-1 arg functions
        if func in ['filter']:
            prev_out = self.filter_(param) if param is not None else self.filter_(prev_out)
