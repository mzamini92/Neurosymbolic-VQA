...
sub_q_type_idx = 7
nb_questions = 10
# Possible Answers: [yes, no, 'rectangle', 'circle', 'triangle', 'square', 'pentagon', 'hexagon', 'octagon', 'star', 1, 2, 3, 4, 5, 6, 7, 8]
colors = [
    (0,0,255), # red
    (0,255,0), # green
    (255,0,0), # blue
...
shapes = ['rectangle', 'circle', 'triangle', 'square', 'pentagon', 'hexagon', 'octagon', 'star']
...
def build_sample():
    ...
    for color_id, color in enumerate(colors):
        ...
        shape = random.choice(shapes) # Randomly select a shape from the updated list

        if shape == 'rectangle':
            start = (center[0]-size, center[1]-(2*size))
            end = (center[0]+size, center[1]+(2*size))
            ...
            points = np.array([(center[0], center[1] + size),
                              (center[0] - size_2//2, center[1]),
                              (center[0] + size_2//2, center[1])])
            cv2.fillPoly(img, [points], color)
            objects.append((color_id, center, 't', (start[0], start[1], end[0], end[1]))) 
            
        elif shape == 'square':
            start = (center[0]-size, center[1]-size)
            end = (center[0]+size, center[1]+size)
            cv2.rectangle(img, start, end, color, -1)
            objects.append((color_id, center, 'q', (start[0], start[1], end[0], end[1]))) 
            ...
            points = np.array([(center[0], center[1] - size),
                              ...
                              (center[0] + size, center[1] - int(size / 2))])
            cv2.fillPoly(img, [points], color)
            objects.append((color_id, center, 'p', (start[0], start[1], end[0], end[1]))) 
            
        elif shape == 'hexagon':
            start = (center[0]-size, center[1]-size)
            end = (center[0]+size, center[1]+size)
            # Define the coordinates of a hexagon
            points = np.array([(center[0] - size, center[1]),
                              ...
                              (center[0] - int(size / 2), center[1] + int(size * 0.86))])
            cv2.fillPoly(img, [points], color)
            objects.append((color_id, center, 'h', (start[0], start[1], end[0], end[1]))) 
        elif shape == 'octagon':
            start = (center[0]-size, center[1]-size)
            end = (center[0]+size, center[1]+size)
            # Define the coordinates of an octagon
            points = np.array([(center[0] - size, center[1]),
                              ...
                              (center[0] - size, center[1] + int(size / 2))])
            cv2.fillPoly(img, [points], color)
            objects.append((color_id, center, 'o', (start[0], start[1], end[0], end[1]))) 
        elif shape == 'star':
            start = (center[0]-size, center[1]-size)
            end = (center[0]+size, center[1]+size)
            ...
            points = np.array([(center[0], center[1] - size),
                              ...
                              (center[0] + int(size * 0.224), center[1] - int(size * 0.224))])
            cv2.fillPoly(img, [points], color)
            objects.append((color_id, center, 's', (start[0], start[1], end[0], end[1])))
    ...
    for _ in range(nb_questions):
        ...
        #print(objects[color],'objects[color]')
        if subtype == 0:
            # query shape -> rectangle/circle
            #print(objects[color][2],'objects[color][2]')
            if objects[color][2] == 'r':
                answer = 2
            elif objects[color][2] == 'c':
                answer = 3
            elif objects[color][2] == 't':
                answer = 4
            elif objects[color][2] == 'q':
                answer = 5
            elif objects[color][2] == 'p':
                answer = 6
            elif objects[color][2] == 'h':
                answer = 7
            elif objects[color][2] == 'o':
                answer = 8
            else:
                answer = 9
    ...
    for _ in range(nb_questions):
        ...
        if subtype == 0:
            ...
            if objects[idx][2] == 'r':
                answer = 2
            elif objects[idx][2] == 'c':
                answer = 3
            elif objects[idx][2] == 't':
                answer = 4
            elif objects[idx][2] == 'q':
                answer = 5
            elif objects[idx][2] == 'p':
                answer = 6
            elif objects[idx][2] == 'h':
                answer = 7
            elif objects[idx][2] == 'o':
                answer = 8
            else:
                answer = 9
...
def convert_sample(sample):
    ...
    answer_sheet = ['yes', 'no', 'rectangle', 'circle', 'triangle', 'square', 'pentagon', 'hexagon', 'octagon', 'star', '1', '2', '3', '4', '5', '6', '7','8']
    ...
    for i, (question, answer) in enumerate(zip(questions, answers)):
        ...
        # Relational questions
            ...
            color = colors[question.tolist()[:6].index(1)]
            obj_shape = objects[colors.index(color)][2]
            if obj_shape == 'r':
                d = 'rectangle'
            elif obj_shape == 'c':
                d = 'circle'
...
def build_dataset(num_samples, data_dir, prefix='train'):
    ...
    shape_map = {'r': 'rectangle', 'c': 'circle','t': 'triangle', 'q': 'square', 'p': 'pentagon', 'h': 'hexagon', 'o': 'octagon', 's': 'star'}
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='NSAI Dataset Generator')
